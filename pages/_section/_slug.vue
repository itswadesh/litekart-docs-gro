<template>
  <div class="-mx-4 lg:mx-0 flex flex-col-reverse lg:flex-row">
    ...
    >
      ...
      >
        ⚠️ You are looking at the english version of the page. Help us translate
        it <a :href="docLink" class="text-orange-600">here</a>.
      </div>
      ...
      <article v-else>
        ...
        <AppResponsiveVideo
          v-if="page.attrs.youtube"
          :src="page.attrs.youtube"
        />
...
export default {
  async asyncData({ $docs, params, store, error, app }) {
    ...
    try {
      ...
      data.contributors = (
        await fetch('https://contributors-api.onrender.com' + path).then(res =>
          res.json()
        )
      ).map(({ author }) => ({ author }))
      if (!page.attrs.title) {
        console.error(
          `[/${path}] ${store.state.lang.text.please_define_title}.`
        )
      } // eslint-disable-line no-console
      if (!page.attrs.description) {
        console.error(
          `[/${path}] ${store.state.lang.text.please_define_description}.`
        )
      } // eslint-disable-line no-console
      data.page = page
    } catch (err) {
      if (!err.response || err.response.status !== 404) {
        return error({
          statusCode: 500,
          message: store.state.lang.text.an_error_occurred
        })
      }
      return error({
        statusCode: 404,
      ...
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: this.page.attrs.description
        },
        // Open Graph
        {
          hid: 'og:title',
          property: 'og:title',
          content: this.page.attrs.title
        },
        {
          hid: 'og:description',
